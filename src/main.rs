...
impl Default for DozenalCalcApp {
    fn default() -> Self {
        Self { 
            ...
            cursor_pos: 0,
            memory: Vec::new(), // <--- NEU: Startet leer
        }
    }
}
...
impl DozenalCalcApp {
    fn calculate_result(&mut self) {
        ...
        // 1. Eingabe-Tokens in mathematische Strings wandeln
        for token in &self.input_buffer {
            match token {
                ...
                _ => {
                    // Zahl abschließen (Dozenal -> Dezimal-Bruch)
        ...
        // Letzte Zahl im Puffer verarbeiten
        if !int_digits.is_empty() || !frac_digits.is_empty() {
            let int_val = if int_digits.is_empty() { "0".to_string() } else { DozenalConverter::to_decimal(&int_digits).to_string() };
            if in_fraction && !frac_digits.is_empty() {
                let frac_val = DozenalConverter::to_decimal(&frac_digits).to_string();
        ...
        // 2. Pre-Parser für Spezial-Operationen
                ...
                if self.input_buffer.is_empty() {
                    ...
                    for token in self.result_buffer.iter().rev() {
                        ...
                        match token {
                            ...
                            CalcToken::Sub => {
                                ui.painter().line_segment([rect.left_center(), rect.right_center()], Stroke::new(2.5, Color32::WHITE));
                            },
                            CalcToken::Decimal => {
                                ui.painter().circle_filled(rect.center_bottom() + Vec2::new(12.0, -5.0), 3.0, Color32::WHITE);
                            },
                            _ => {}
                        }
                            ...
                            match token {
                                ...
                                _ => {
                                    let text = match token {
                                        ...
                                        CalcToken::ParenOpen => "(", CalcToken::ParenClose => ")",
                                        CalcToken::Sin => "sin", CalcToken::Cos => "cos",
                                        CalcToken::Tan => "tan", CalcToken::Cot => "cot",
